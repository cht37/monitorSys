...
import com.neu.monitor_sys.statistics.client.FeedbackClient;
import com.neu.monitor_sys.statistics.client.GeoClient;
import com.neu.monitor_sys.statistics.client.UserClient;
...
import com.neu.monitor_sys.statistics.publisher.StatisticsPublisher;
import com.neu.monitor_sys.statistics.repository.EsStatisticsRepository;
import com.neu.monitor_sys.statistics.service.IStatisticsService;
import com.neu.monitor_sys.statistics.utils.StatisticsUtil;
...
//    }
    ...
    MyResponse<int[]> response = aqiClient.getMinValueByLevel(level);
    if (response.getStatusCode() != ResultCode.SUCCESS.getCode()) {
        throw new RuntimeException("获取污染限值失败，状态码：" + response.getStatusCode());
    }
    int[] minValues = response.getData();
    int so2Min = minValues[0];
    int coMin = minValues[1];
    ...
    // 时间范围过滤器
    QueryBuilder timeFilter = QueryBuilders.rangeQuery("confirm_datetime")
            .from(fourWeeksAgo.format(formatter))
    ...
    // 计算超标次数
    FilterAggregationBuilder so2ExceedFilter = createExceedFilter("so2_value", so2Min);
    FilterAggregationBuilder coExceedFilter = createExceedFilter("co_value", coMin);
    FilterAggregationBuilder spmExceedFilter = createExceedFilter("spm_value", spmMin);

    // 添加聚合
    provinceAgg.subAggregation(so2ExceedFilter);
    provinceAgg.subAggregation(coExceedFilter);
    provinceAgg.subAggregation(spmExceedFilter);
    ...
    for (Terms.Bucket provinceBucket : provinceTerms.getBuckets()) {
        String provinceId= provinceBucket.getKeyAsString();

        PollutionStatisticsDTO dto = new PollutionStatisticsDTO();
        dto.setProvinceId(provinceId);

        Filter so2Exceed = provinceBucket.getAggregations().get("so2_value_exceed_filter");
        long so2ExceedCountValue = ((ValueCount) so2Exceed.getAggregations().get("so2_value_exceed_count")).getValue();
        dto.setSo2ExceedTimes((int) so2ExceedCountValue);

        Filter coExceed = provinceBucket.getAggregations().get("co_value_exceed_filter");
        long coExceedCountValue = ((ValueCount) coExceed.getAggregations().get("co_value_exceed_count")).getValue();
        dto.setCoExceedTimes((int) coExceedCountValue);

        Filter spmExceed = provinceBucket.getAggregations().get("spm_value_exceed_filter");
        long spmExceedCountValue = ((ValueCount) spmExceed.getAggregations().get("spm_value_exceed_count")).getValue();
        dto.setSpmExceedTimes((int) spmExceedCountValue);

        stats.add(dto);
    }
    for (PollutionStatisticsDTO stat : stats) {
        String provinceName = (String) geoClient.getProvinceName(stat.getProvinceId()).getData();
        stat.setProvinceName(provinceName);
    }
...
private FilterAggregationBuilder createExceedFilter(String field, int minValue) {
    return AggregationBuilders.filter(field + "_exceed_filter",
            QueryBuilders.rangeQuery(field).gte(minValue))
            .subAggregation(AggregationBuilders.count(field + "_exceed_count").field(field));
}
