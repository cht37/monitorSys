package com.neu.monitor_sys.aqi.service.impl;

import cn.hutool.json.JSONUtil;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.neu.monitor_sys.aqi.mapper.AqiMapper;
import com.neu.monitor_sys.aqi.service.IAqiService;
import com.neu.monitor_sys.aqi.util.RedisUtil;
import com.neu.monitor_sys.common.DTO.AqiDTO;
import com.neu.monitor_sys.common.constants.RedisPrefix;
...
public class AqiServiceImpl extends ServiceImpl<AqiMapper, Aqi> implements IAqiService {
    ...
    public int[] calculateAqi(int so2, int co, int spm) {
        ...
        //判断是否超最大范围
        if (so2 > 2620 || co > 60 || spm > 421||so2<0||co<0||spm<0) {
            throw new IllegalArgumentException("超出最大范围");
        }
    ...
    private String[] getRange(String aqiExplain) {
        if (aqiExplain.equals("300+")){
            return new String[]{"300","9999"};
        }
    ...
    public int interpolateAqi(int concentration, int lowConcentration, int highConcentration, String[] aqiRange) {
        ...
        // 根据公式计算AQI值
        return (highAqi - lowAqi) * (concentration - lowConcentration) / (highConcentration - lowConcentration) + lowAqi;
    }
}
